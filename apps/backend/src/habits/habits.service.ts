...
export class HabitsService {
    ...
    async update(id: string, updateHabitDto: UpdateHabitDto, userId: string): Promise<Habit> {
        const habit = await this.findOne(id, userId);
        Object.assign(habit, updateHabitDto);
        return this.habitsRepository.save(habit);
    }

    async logHabit(habitId: string, userId: string, date: string, count: number = 1): Promise<HabitLog> {
    ...
    private async updateStreaks(habit: Habit): Promise<void> {
        ...
        if (latestLogDate !== todayStr && latestLogDate !== yesterdayStr) {
            ...
            for (let i = 0; i < logs.length - 1; i++) {
                ...
                const diffDays = Math.round((currentLogDate.getTime() - nextLogDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    async getStatistics(userId: string) {
        ...
        const totalCompletions = habits.reduce((acc, h) => acc + h.logs.filter(l => l.is_completed).length, 0);
        ...
        const monthDays = eachDayOfInterval({ start, end }).map(date => format(date, 'yyyy-MM-dd'));
