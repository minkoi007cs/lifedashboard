...
export class WishesService {
  constructor(
    ...
    @InjectRepository(WishComment)
    private wishCommentsRepository: Repository<WishComment>,
  ...
  async getFeed(userId: string) {
    ...
    return shares.map((share) => {
      ...
      const currentResponse =
        wish.responses.find((response) => response.responderId === userId) ??
        null;
      ...
      return {
        ...
        hasUpdatesSinceResponse:
          !!currentResponse && wish.updatedAt > currentResponse.respondedAt,
  ...
  async update(wishId: string, updateWishDto: UpdateWishDto, ownerId: string) {
    ...
    if (wish.planCreatedAt) {
      throw new BadRequestException(
        'Wish can no longer be edited after a plan has been created',
      );
    }
    ...
    const recipientIds = (savedWish.shares ?? []).map(
    ...
    return this.getWishForOwner(savedWish.id, ownerId).then((entry) =>
      this.serializeOwnerWish(entry),
    );
  }
  ...
  async remove(wishId: string, ownerId: string) {
    ...
    if (wish.planCreatedAt) {
      throw new BadRequestException(
        'Wish with a created plan cannot be deleted',
      );
    }
  ...
  async share(wishId: string, shareWishDto: ShareWishDto, ownerId: string) {
    const wish = await this.getWishForOwner(wishId, ownerId);
    const targetUserIds = [...new Set(shareWishDto.userIds)].filter(
      (id) => id !== ownerId,
    );
    ...
    const removedRecipientIds = existingRecipientIds.filter(
      (recipientId) => !targetUserIds.includes(recipientId),
    );
    if (
      removedRecipientIds.some((recipientId) =>
        respondedUserIds.has(recipientId),
      )
    ) {
      throw new BadRequestException(
        'Cannot unshare a wish from users who have already responded',
      );
    }

    const targetUsers = await Promise.all(
      targetUserIds.map((userId) => this.usersService.findById(userId)),
    );
    ...
    const newRecipientIds = targetUserIds.filter(
      (recipientId) => !existingRecipientIds.includes(recipientId),
    );
    if (newRecipientIds.length > 0) {
      const shares = newRecipientIds.map((recipientId) =>
        this.wishSharesRepository.create({
          wishId,
          recipientId,
        }),
      );
    ...
    return this.getWishForOwner(wishId, ownerId).then((entry) =>
      this.serializeOwnerWish(entry),
    );
  }
  ...
  async respond(
    wishId: string,
    respondToWishDto: RespondToWishDto,
    responderId: string,
  ) {
    ...
    if (wish.ownerId === responderId) {
      throw new ForbiddenException(
        'Wish owner cannot respond to their own wish',
      );
    }

    const isSharedRecipient = (wish.shares ?? []).some(
      (share) => share.recipientId === responderId,
    );
    if (!isSharedRecipient) {
      throw new ForbiddenException(
        'You do not have access to respond to this wish',
      );
    }
    ...
    if (wish.planCreatedAt) {
      if (!existingResponse) {
        throw new BadRequestException(
          'Cannot create a new response after the plan has been created',
        );
      }
    ...
    if (
      respondToWishDto.status === WishResponseStatus.CONFIRMED &&
      wish.type !== WishType.ACTIVITY &&
      respondToWishDto.addToPlan
    ) {
      throw new BadRequestException(
        'Only activity wishes can be added to a plan',
      );
    }

    const response =
      existingResponse ??
      this.wishResponsesRepository.create({
        wishId,
        responderId,
      });
  ...
  async addComment(
    wishId: string,
    createWishCommentDto: CreateWishCommentDto,
    userId: string,
  ) {
    const wish = await this.wishesRepository.findOne({
      where: { id: wishId },
    ...
    const canAccess =
      wish.ownerId === userId ||
      (wish.shares ?? []).some((share) => share.recipientId === userId);
    if (!canAccess) {
      throw new ForbiddenException(
        'You do not have access to comment on this wish',
      );
    }
    ...
    if (author) {
      const recipientIds = new Set<string>([
        wish.ownerId,
        ...(wish.shares ?? []).map((share) => share.recipientId),
      ]);
  ...
  async createPlanFromWish(
    wishId: string,
    createPlanDto: CreatePlanFromWishDto,
    ownerId: string,
  ) {
    const wish = await this.getWishForOwner(wishId, ownerId);
    if (wish.type !== WishType.ACTIVITY) {
      throw new BadRequestException(
        'Only activity wishes can be turned into plans',
      );
    }
    if (wish.planCreatedAt) {
      throw new BadRequestException(
        'Plan has already been created for this wish',
      );
    }
    ...
    const participantIds = (wish.responses ?? [])
      .filter(
        (response) =>
          response.status === WishResponseStatus.CONFIRMED &&
          response.addToPlan,
      )
    ...
    if (participantIds.length === 0) {
      throw new BadRequestException(
        'At least one confirmed participant must opt in to the plan',
      );
    }
  ...
  private serializeOwnerWish(wish: WishEntry) {
    const responseSummary = (wish.responses ?? []).reduce<OwnerWishSummary>(
      (summary, response) => {
