    ...
    return (
        <>
            <WidgetFrame
                ...
                footer={
                    <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center rounded-2xl bg-orange-50/80 px-4 py-3 text-sm font-semibold text-pink-600 dark:bg-slate-800/90 dark:text-pink-300">
            ...
            >
                <div className="space-y-3 overflow-y-auto flex-1">
                    {habits?.map((habit) => {
                        const isDoneToday = habit.logs?.some(log => log.date === today) ?? false;
                        return (
