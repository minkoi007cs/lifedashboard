...
### 4.10. Smart Mail Hub & Model Context Protocol (MCP) Server
* **`ld_mail_accounts`**: Quản lý đa tài khoản Email (id, user_id, provider: `gmail` | `outlook`, email, label, color, access_token, refresh_token, sync_status: `active` | `error` | `syncing`, last_synced_at, created_at). Cho phép người dùng kết nối nhiều tài khoản cùng lúc (ví dụ: Gmail cá nhân, Gmail công việc, Outlook trường ĐH, Outlook công ty) qua Onboarding Setup Wizard.
* **`ld_mail_messages`**: Dữ liệu email đồng bộ & phân tích AI (id, user_id, account_id, external_id, thread_id, from_name, from_address, to_address, subject, snippet, body, received_at, is_read, is_starred, category: `ALL` | `ACTION_REQUIRED` | `ACADEMIC` | `WORK` | `FINANCE` | `NEWSLETTER` | `PROMOTIONS` | `SPAM` | `PERSONAL`, ai_summary, ai_action_items jsonb, ai_draft_reply, ai_priority: `urgent` | `high` | `medium` | `low`, ai_action_required bool, extracted_amount, is_task_converted, linked_task_id, is_expense_converted, linked_transaction_id).
* **MCP Server Integration (`/api/v1/mail/mcp`)**: Cung cấp giao thức Model Context Protocol chuẩn với 7 công cụ AI: `list_mail_accounts`, `fetch_unread_emails`, `summarize_email`, `classify_email`, `clean_spam`, `convert_to_task`, `draft_reply` cho phép các Agent (Claude Desktop, Cursor, Antigravity) quản lý hòm thư tự hành.
* **Outlook & Gmail Live Reader**: Nhận và phát hiện thư mới đến, kích hoạt AI tóm tắt ngắn và đẩy thông báo tức thì lên hệ thống qua `ld_notifications`.
